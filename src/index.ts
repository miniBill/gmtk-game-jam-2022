import { Elm, MainInstance } from "./Main.elm";

function addGamepadPort(elmApp: MainInstance) {
  ...
  var previousFrame: {
    gamepads: { axes: any; buttons: any; id: any; index: any; mapping: any }[];
    timestamp: number;
  };
    ...
    for (var i = 0; i < rawGamepads.length; i++) {
      ...
      if (g && g.connected && g.timestamp > 0)
        serialisedGamepads.push({
          axes: g.axes,
          buttons: g.buttons.map(function (b: { pressed: any; value: any }) {
